...
exports.updateMajor = async function (req, res) {
    try {
        const id = req.params.id
        checkIdParam(id) //throws an error if id is not a number
        if (!(await majorService.updateMajor(id, req.body))) throw new InvalidParamError(MAJOR, id)
...
exports.deleteMajorById = async function (req, res) {
    try {
        const id = req.params.id
        checkIdParam(id) //throws an error if id is not a number
        if (!(await majorService.deleteMajorById(id))) throw new InvalidParamError(MAJOR, id)
